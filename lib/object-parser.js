...
exports.parseHotel = function (input) {
	const parseObj = item => {
...
exports.parseHotelRoom = function (input) {
	const parseObj = item => {
		const r = _.pick(
			item,
			'hotel',
...
exports.parseTravelPackage = function (input) {
	const parseObj = item => {
		...
		r.id = item._id;
		r.imageUrl = item.image.secure_url;
		r.titleImageUrl = item.titleImage.secure_url;
		r.carouselImages = _.map(item.carouselImages || [], img => {
