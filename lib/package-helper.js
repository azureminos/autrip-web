...
const {Instance, Global, Modal} = CONSTANTS.get();
...
const enhanceInstance = ({userId, instPackage, rates}) => {
  ...
  const defaultPeople = isOwner ? Global.defaultPeople : 0;
  const defaultRooms = isOwner ? Global.defaultRooms : 0;
  const extras = {
    statusMember: member.status,
    isOwner: isOwner,
    min: 999,
    max: 0,
    people: member.people ? member.people : defaultPeople,
    rooms: member.rooms ? member.rooms : defaultRooms,
    otherRooms: 0,
    otherPeople: 0,
  };
  // Set min & max
...
const validateInstance = (instPackage, instPackageExt) => {
  console.log('>>>>PackageHelper.validateInstance', {
    instPackage,
    instPackageExt,
  });
  if (!instPackage.startDate) {
    return Modal.INVALID_DATE.key;
  } else if (instPackage.totalPeople < instPackageExt.min) {
    return Modal.LESS_THAN_MIN.key;
  } else if (instPackage.totalPeople > instPackageExt.max) {
    return Modal.INVALID_MAX_PARTICIPANT.key;
  } else if (instPackage.status !== Instance.status.IN_PROGRESS) {
    return Modal.INVALID_STATUS.key;
  }
  return null;
};

const getCityImage = (itinerary, cities) => {
  console.log('>>>>PackageHelper.getCityImage', {itinerary, cities});
  if (itinerary.attractions) {
    ...
    } else if (itinerary.attractions && itinerary.attractions.length > 0) {
